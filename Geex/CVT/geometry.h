...
namespace Geex {
	...
	template <class T> inline void seg_centroid(
		const vec3g<T>& p, const vec3g<T>& q, vec3g<T>& g, T& V
		) {
			V = length(p-q) ;
			g = T(1.)/T(2.) * (p+q) ;
	}
	...
	template <class T> inline void seg_centroid(
		const vec3g<T>& p, const vec3g<T>& q, T a, T b, vec3g<T>& g, T& V
	) {
		T ab = a + b;
		T l = length(p - q);
		V = l / T(2)*ab;
		g = (l/T(6))*((a+ab)*p+(b+ab)*q);
		g /= V;
	}
