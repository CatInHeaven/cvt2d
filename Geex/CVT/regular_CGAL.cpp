...
//		typedef CGAL::Regular_triangulation_euclidean_traits_3<K>    Traits;
		typedef CGAL::Regular_triangulation_cell_base_3<K, Cell<K> > Cb;
		typedef Vertex<K> Vb ;
//		typedef CGAL::Regular_triangulation_vertex_base_3<K>         Vb;
		typedef CGAL::Triangulation_data_structure_3<Vb,Cb> TDS;
		typedef CGAL::Regular_triangulation_3<K, TDS>          TRI; 
...
//		typedef Traits::Weighted_point_3                              Weighted_point;
		...
		inline vec3 to_geex(const Weighted_point& p) {
			return vec3(
				p.cartesian(0), p.cartesian(1), p.cartesian(2)
			);
		}
	...
	void Regular_CGAL::set_vertices(unsigned int n, const double* xyz) {
		...
		////		v = impl_->insert(impl_->sorted_points_[i].pt, ch, 0);
	...
	void Regular_CGAL::get_is_hidden(std::vector<bool>& hidden) {
		for(Implementation::Finite_vertices_iterator vi = impl_->finite_vertices_begin(); vi != impl_->finite_vertices_end(); ++vi) {
//			hidden.push_back(vi->is_hidden()) ;
		}
	}
