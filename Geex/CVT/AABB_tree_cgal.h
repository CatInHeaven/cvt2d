    ...
    public:
		...
		void insert(Map* map) {
			...
			FOR_EACH_FACET(Map, map, f){
				...
				triangles_.push_back(CGAL_Triangle(v[0], v[1], v[2])) ;
			}
...
//				tris.push_back(Triangle(v[0], v[1], v[2])) ;
		...
		bool closest_point(const vec3& o, const vec3& dir, vec3& cp, int& fid) {
			...
			CGAL_Vector v = to_cgal(dir)-CGAL::ORIGIN ;
			CGAL_Point  p = to_cgal(o) ;
			CGAL_Ray ray_query(p, v);
	...
	protected:
		std::vector<CGAL_Triangle> triangles_ ;
    } ;
