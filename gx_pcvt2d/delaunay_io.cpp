	...
	void DelaunayIO::save_vertex_degree(const std::string& fname) {
		...
		for(unsigned int i=0; i<vertices.size(); ++i) {
			...
			if(delaunay_->is_primary(vertices[i])) {
				...
				Delaunay::Vertex_circulator cir = delaunay_->adjacent_vertices(vertices[i]) ;
				do {
					...
					etotal ++ ;
					++cir ;
				} while(cir!=delaunay_->adjacent_vertices(vertices[i])) ;
				vtotal ++ ;
			}
		}
	...
	static void min_max_edge_length(Delaunay* dt, double& minlen, double& maxlen) {
		...
		for(unsigned int i=0; i<vertices.size(); ++i) {
			if(dt->is_primary(vertices[i])) {
				Delaunay::Vertex_circulator cir = dt->adjacent_vertices(vertices[i]) ;
				do {
					...
					++cir  ;
				} while(cir!=dt->adjacent_vertices(vertices[i])) ;
			}
		}	
	}
